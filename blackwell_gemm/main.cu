...
// gridDim 1
// blockDim 32
__global__ void gemm_kernel(const u_int32_t *dA, const u_int8_t *dA_Scale,
                            const u_int32_t *dB, const u_int8_t *dB_Scale,
                            float *dC, int M, int N, int K) {
    const int idx = threadIdx.x;
    u_int32_t a_fp4[4];
    u_int32_t b_fp4[2];
    u_int32_t scaleA = 0x00000000;
    u_int32_t scaleB = 0x00000000;
    ...
    asm volatile("mma.sync.aligned.m16n8k64.row.col.kind::mxf4.block_scale.scale_vec::2X"
                ...
                : "r"(a_fp4[0]), "r"(a_fp4[1]), "r"(a_fp4[2]), "r"(a_fp4[3]),
                  "r"(b_fp4[0]), "r"(b_fp4[1]), "f"(C[0]), "f"(C[1]), "f"(C[2]),
                  "f"(C[3]), "r"(scaleA), "r"(scaleB));

    // Store results back to global memory
    dC[(A_row_id + 0) * N + 2 * B_row_id + 0] = D[0];
    dC[(A_row_id + 0) * N + 2 * B_row_id + 1] = D[1];
    dC[(A_row_id + 8) * N + 2 * B_row_id + 0] = D[2];
    dC[(A_row_id + 8) * N + 2 * B_row_id + 1] = D[3];
}
...
void initScaleMatrix(u_int8_t *scaleMatrix, const int scaleSize) {
    ...
    scaleMatrix[i] =
        rand() % 6 + 124; // Random scale value between 2^-3 and 2^2
    }
}

void matmul_mxfp4_cpu(const u_int32_t *A, const u_int8_t *A_Scale,
                      const u_int32_t *B, const u_int8_t *B_Scale, float *C,
    ...
    for (int n = 0; n < N; n++) {
        ...
        for (int k = 0; k < K; k++) {
            ...
            float a_val = FP4_VALUES.at(a_fp4) *
                            powf(2.0f, A_Scale[m * K / 32 + k / 32] - 127);
            float b_val = FP4_VALUES.at(b_fp4) *
                            powf(2.0f, B_Scale[n * K / 32 + k / 32] - 127);
...
int main(int argc, char **argv) {
    ...
    matmul_mxfp4_cpu(matrixA, matrixA_Scale, matrixB, matrixB_Scale, matrixC, M,
                    N, K);
    ...
    cudaMemcpy(d_A, matrixA, matrixASize * sizeof(u_int32_t),
                cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, matrixB, matrixBSize * sizeof(u_int32_t),
                cudaMemcpyHostToDevice);
    cudaMemcpy(d_A_Scale, matrixA_Scale, matrixAscaleSize * sizeof(u_int8_t),
                cudaMemcpyHostToDevice);
    cudaMemcpy(d_B_Scale, matrixB_Scale, matrixBscaleSize * sizeof(u_int8_t),
                cudaMemcpyHostToDevice);

    // Launch kernel
    dim3 gridDim(1);
    dim3 blockDim(32);
    gemm_kernel<<<gridDim, blockDim>>>(d_A, d_A_Scale, d_B, d_B_Scale, d_C, M, N,
                                        K);
